...
#os.chdir("C:\\Users\\llavin\\Desktop\\dispatch_RA_model")
cwd = os.getcwd()
...
class DirStructure(object):
    ...
    def __init__(self, code_directory, case_folder):
        ...
        self.SCENARIO_INPUTS_DIRECTORY = os.path.join(
            self.INPUTS_DIRECTORY, "case_creation_input"
        )
...
for date in dates:
    ...
    if make_init:
        print(
            "make_init is "
            + str(make_init)
            + " for this case, so it should be the day that initializes your run"
        )
    else:
        print(
            "make_init is "
            + str(make_init)
            + " for this case, so it should draw on a prior case to initialize its dispatch when run"
        )
    ...
    month_rename_list = [
        ...
        "Nov",
        "Dec",
    ]  # this must be same as scheduled.outage.rate.by.gen.type.FULL.PERIOD.032519 month naming convention
    numeric_month = int(
        scenario_name.split(".")[0]
    )  # assumes date will list month first, as is standard in USA
    month = month_rename_list[numeric_month - 1]  # -1 b/c Python lists index from 0

    # pull case-level data
    data = raw_data_imports.load_data(
        os.path.join(dir_str.INPUTS_DIRECTORY),
        os.path.join(dir_str.SCENARIO_INPUTS_DIRECTORY),
        date,
    )

    # run and create the main set of csvs
    data_to_csvs.write_data(
        data,
        dir_str.RESULTS_DIRECTORY,
        make_init,
        dir_str.SCENARIO_INPUTS_DIRECTORY,
        date,
        os.path.join(dir_str.INPUTS_DIRECTORY),
        primary_reserve_scalar,
        secondary_reserve_scalar,
    )

    # create an additional dynamic ORDC file, if desired
    if create_supp_ordc:
        ordc_df = ordc_sandbox.load_and_run_ordc(
            dir_str.INPUTS_DIRECTORY,
            dir_str.RESULTS_DIRECTORY,
            month,
            hydro_cf,
            VOLL,
        ...
        ordc_df.to_csv(
            os.path.join(dir_str.RESULTS_DIRECTORY, "full_ordc.csv"), index=False
        )
    elif PJM_reserve_heuristic:
        print(
            "you have chosen PJM heuristic reserve requirements (will be based on inputs)"
        )
        ordc_df = ordc_sandbox.PJM_reserves(
            dir_str.INPUTS_DIRECTORY,
            dir_str.RESULTS_DIRECTORY,
            n_segments,
            lfe,
        ...
        ordc_df = ordc_sandbox.no_ordc(
            dir_str.INPUTS_DIRECTORY, dir_str.RESULTS_DIRECTORY, n_segments
        )
        ordc_df.to_csv(
            os.path.join(dir_str.RESULTS_DIRECTORY, "full_ordc.csv"), index=False
        )
